...
function App() {
  ...
  const [ModifierPostId, setModifierPostId] = useState(null);
  ...
  const addOrUpdatePost = () => {
    if (IsModifier) {
      const updatedPosts = posts.map(post =>
        post.id === ModifierPostId ? { ...post, title: newPost.title, body: newPost.body } : post
      );
      setPosts(updatedPosts.sort((a, b) => b.id - a.id));
      setFiltredPosts(updatedPosts.sort((a, b) => b.id - a.id));
      setIsModifier(false);
      setModifierPostId(null);
    } else {
      ...
      <form>
        ...
        <div>
          ...
          <button type="button" className="btn btn-success m-2" onClick={addOrUpdatePost}>
            {IsModifier ? "Mettre à jour" : "Ajouter"}
          </button>
        </div>
              ...
              filteredPosts.length > 0 ? filteredPosts.map(post => (
                <tr key={post.id}>
                  ...
                  <td>
                    ...
                    <button type="button" className="btn btn-primary m-2" onClick={() => modifier(post)}>Éditer</button>
                  </td>
                </tr>
